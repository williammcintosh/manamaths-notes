...
\footnotesize
\KeyIdea{Do the inverse (opposite) to both sides.\\
Add \(\leftrightarrow\) Subtract \quad\textbar\quad Multiply \(\leftrightarrow\) Divide}
\vspace{-0.1em}
...
\end{column}
\begin{column}{0.48\textwidth}
\CommonMistake{Forgetting to do the same to both sides.\\
\(x+3=7\) means subtract 3 from both sides, not just move the 3.
For \(x-4=10\) add 4 (not subtract 4). Undo what's been done to \(x\).}
\end{column}
\end{columns}
\end{frame}
